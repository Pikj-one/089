...
@dataclass
class TaskSpec:
    id: str; title: str; description: str; required_output: str = "结构化发现与证据"; relevant_context: str = ""; order: int = 0
    def to_dict(self): return _plain(self)
    @classmethod
    def from_dict(cls, d): return cls(**{k:d[k] for k in ("id","title","description")}, required_output=d.get("required_output","结构化发现与证据"), relevant_context=d.get("relevant_context",""), order=d.get("order",0))
