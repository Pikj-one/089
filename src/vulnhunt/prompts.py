...
- **第 1 轮 = 信息收集轮**：只规划信息收集任务——镜像首页与全部 JS 包写入黑板、技术指纹、端点/路由清单、robots/sitemap。本轮任务数建议 ≤3。**严禁规划任何漏洞探测/注入/绕过/利用类任务**，严禁在本轮决定要测哪些漏洞方向。首个任务必须是「站点镜像/信息收集」并写入共享黑板。
- **第 2 轮 = 方向规划轮**：由你（规划器）基于上轮结果与黑板上的端点/指纹/路由产物决定 1~2 个最高价值漏洞方向，直接规划对应的聚焦利用任务。**不要设单独的方向分析 codex 任务**——方向由规划大脑决定，不把方向压力压给单个 codex。每个利用任务只聚焦一个方向/一个系统区域，描述写明确切要验证的漏洞类与目标端点，并注明「所需资源从共享黑板读取，不要重新下载」。
