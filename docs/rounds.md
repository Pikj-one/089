...
| 轮次 | 阶段 | 规划器做什么 | 禁止 |
|---|---|---|---|
| 第 1 轮 | **信息收集轮** | 只规划信息收集：镜像首页与全部 JS 包写入共享黑板、技术指纹、端点/路由清单、robots/sitemap；**首个任务必须是站点镜像并写入黑板** | 严禁任何漏洞探测/注入/绕过/利用类任务；严禁在无信息时押注漏洞方向 |
| 第 2 轮 | **方向规划轮** | 基于上轮结果 + 黑板上的端点/指纹/路由产物，决定 5~8 个最高价值漏洞方向，直接规划对应的聚焦利用任务 | 禁止设单独的方向分析 codex 任务（方向由规划大脑决定）；禁止一次性铺开所有方向 |
...
`max_rounds` 只是硬上限（代码默认 5，本项目 `config.toml` 设为 50），实际轮数由规划器按阶段自主推进决定。正常节奏约 3~5 轮：1 轮信息收集 → 1 轮定方向并开跑 → 数轮深化利用。
